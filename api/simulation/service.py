...
async def _read_within_cap(file: UploadFile, max_bytes: int) -> bytes:
    ...
    while True:
        ...
        if len(buffer) + len(chunk) > max_bytes:
            raise ErrorException(error=errors.simulation.FILE_TOO_LARGE, status_code=413)
        buffer.extend(chunk)
    return bytes(buffer)
...
class SimulationService:
    ...
    async def start_simulation(
        ...
        # 2. Read the upload body, bounded at UPLOAD_MAX_BODY_BYTES so a chunked
        # request (no Content-Length, unscreened by the middleware) can't exhaust
        # memory here. Reject empty files so the worker doesn't waste a slot on a
